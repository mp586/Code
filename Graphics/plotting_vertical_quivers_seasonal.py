...
#print control_dir
ctl_runmin=361  # Should be a January month for seasonal variables to be correct
ctl_runmax=481
...
wcomp1_seasonal_avg_ctl = - (omega1_seasonal_avg_ctl * temp1_seasonal_avg_ctl * Rspec)/(pfull * g)
wcomp2_seasonal_avg_ctl = - (omega2_seasonal_avg_ctl * temp2_seasonal_avg_ctl * Rspec)/(pfull * g)
wcomp1_seasonal_avg = - (omega1_seasonal_avg * temp1_seasonal_avg * Rspec)/(pfull * g)
wcomp2_seasonal_avg = - (omega2_seasonal_avg * temp2_seasonal_avg * Rspec)/(pfull * g)
wcomp3_seasonal_avg_ctl = - (omega3_seasonal_avg_ctl * temp3_seasonal_avg_ctl * Rspec)/(pfull * g)
wcomp3_seasonal_avg = - (omega3_seasonal_avg * temp3_seasonal_avg * Rspec)/(pfull * g)
...
for season in seasons:
    ...
    uwind = (ucomp1_seasonal_avg - ucomp1_seasonal_avg_ctl).sel(season=season)
    wwind = ((wcomp1_seasonal_avg - wcomp1_seasonal_avg_ctl).sel(season=season)*8000.)[::-1,:,:]
    array = (rh1_seasonal_avg - rh1_seasonal_avg_ctl).sel(season=season)
    lons = uwind.lon 
    lats = uwind.lat
    pres = wwind.pres_lev
    ...
    uwind = (ucomp3_seasonal_avg - ucomp3_seasonal_avg_ctl).sel(season=season)
    wwind = ((wcomp3_seasonal_avg - wcomp3_seasonal_avg_ctl).sel(season=season)*8000.)[::-1,:,:]
    array = (rh3_seasonal_avg - rh3_seasonal_avg_ctl).sel(season=season)
    lons = uwind.lon
    lats = uwind.lat
    pres = wwind.pres_lev
    presar = array.pres_lev # need separate z coords for winds because read them in in reverse z order to flip axis
    ...
    uwind = (ucomp2_seasonal_avg - ucomp2_seasonal_avg_ctl).sel(season=season)
    wwind = ((wcomp2_seasonal_avg - wcomp2_seasonal_avg_ctl).sel(season=season)*8000.)[::-1,:,:]
    array = (rh2_seasonal_avg - rh2_seasonal_avg_ctl).sel(season=season)
    lons = uwind.lon
    lats = uwind.lat
    pres = wwind.pres_lev
    presar = array.pres_lev # need separate z coords for winds because read them in in reverse z order to flip axis
    ...
    uwind = ((ucomp2_seasonal_avg - ucomp2_seasonal_avg_ctl) - (ucomp1_seasonal_avg - ucomp1_seasonal_avg_ctl)).sel(season=season)[::-1,:,:]
    wwind = (((wcomp2_seasonal_avg - wcomp2_seasonal_avg_ctl) - (wcomp1_seasonal_avg - wcomp1_seasonal_avg_ctl)).sel(season=season)*8000.)[::-1,:,:]
    array = ((rh2_seasonal_avg - rh2_seasonal_avg_ctl) - (rh1_seasonal_avg - rh1_seasonal_avg_ctl)).sel(season=season)
    lons = uwind.lon
    lats = uwind.lat
    pres = wwind.pres_lev
    presar = array.pres_lev # need separate z coords for winds because read them in in reverse z order to flip axis
    ...
    ### For paper just direcly save the plot at output
    fig.savefig('/scratch/mp586/Code/Graphics/Isca/square_South_America_newbucket_fixedSSTs_from_realworld_zonallysymm_plus_uniform_warming_and_2xCO2_spinup_361/w8000_quivers_4cases_deltarh_saved_360-480_10S-10N_lgefonts_nonshift_'+season+'.png')
    fig.savefig('/scratch/mp586/Code/Graphics/Isca/square_South_America_newbucket_fixedSSTs_from_realworld_zonallysymm_plus_uniform_warming_and_2xCO2_spinup_361/w8000_quivers_4cases_deltarh_saved_360-480_10S-10N_lgefonts_nonshift_'+season+'.svg')
