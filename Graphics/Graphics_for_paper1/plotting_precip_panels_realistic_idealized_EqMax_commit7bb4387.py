...
HPC = 'yes'
control_dir = 'full_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_commit7bb4387'
...
HPC = 'yes'
testdir_in1= 'full_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_plus_2pt52K_and_2xCO2_spinup_361_commit7bb4387'
dire = testdir_in1
runmin=120
runmax=480
if (HPC == 'Yes') or (HPC == 'yes') or (HPC == 'y'):
...
HPC = 'yes'
control_dir = 'two_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_commit7bb4387'
if (HPC == 'Yes') or (HPC == 'yes') or (HPC == 'y'):
    control_dir= control_model + '/ISCA_HPC/' + control_dir
else: 
    control_dir= control_model + '/' + control_dir
...
HPC = 'yes'
testdir_in1= 'two_continents_newbucket_fixedSSTs_zonally_symmetric_EqMax_plus_2pt52K_and_2xCO2_spinup_361_commit7bb4387'
runmin=120
runmax=480
if (HPC == 'Yes') or (HPC == 'yes') or (HPC == 'y'):
...
landmaskid=landfile.variables['land_mask'][:]
landlats=landfile.variables['lat'][:]
landlons=landfile.variables['lon'][:]
# for specified lats
landmaskxrid=xr.DataArray(landmaskid,coords=[landlats,landlons],dims=['lat','lon']) # need this in order to use .sel(... slice) on it
...
nmb_contours_real = [-1.,1.]
nmb_contours_ideal = [-1.,1.]
...
landmaskid = np.asarray(landmaskxrid)


fig, axes = plt.subplots(1, 2, figsize = (20, 8))

axes[0].set_title('a) RC EquatorMax', size = med)
#fig = plt.figure()
...
cont = m.contour(xi,yi,ctl_array,nmb_contours_real, colors = 'k', linewidth=2) # if nmb_contours is not an int, it can be interpreted as an array specifying the contour levels
...
axes[1].set_title('b) 2C EquatorMax', size = med)
#fig = plt.figure()
...
cont = m.contour(xi,yi,ctl_array,nmb_contours_ideal, colors = 'k', linewidth=2) # if nmb_contours is not an int, it can be interpreted as an array specifying the contour levels
...
fig.savefig('/scratch/mp586/Code/Graphics/Isca/ISCA_HPC/'+dire+'/Pavg_minus_ctl_2panels_RCEqmax_vs_2CEqmax_P-Econts_120-480.png', bbox_inches='tight', dpi=100)
fig.savefig('/scratch/mp586/Code/Graphics/Isca/ISCA_HPC/'+dire+'/Pavg_minus_ctl_2panels_RCEqmax_vs_2CEqmax_P-Econts_120-480.svg', bbox_inches='tight', dpi=100)
fig.savefig('/scratch/mp586/Code/Graphics/Isca/ISCA_HPC/'+dire+'/Pavg_minus_ctl_2panels_RCEqmax_vs_2CEqmax_P-Econts_120-480.pdf', bbox_inches='tight', dpi=400)
