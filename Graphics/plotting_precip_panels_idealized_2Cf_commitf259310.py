...
HPC = 'yes'
control_dir = 'two_continents_flipped_newbucket_fixedSSTs_from_realworld_zonallysymm_commitf259310'
if (HPC == 'Yes') or (HPC == 'yes') or (HPC == 'y'):
    control_dir= control_model + '/ISCA_HPC/' + control_dir
else: 
    control_dir= control_model + '/' + control_dir
...
HPC = 'yes'
testdir_in1= 'two_continents_flipped_newbucket_fixedSSTs_from_realworld_zonallysymm_plus_uniform_warming_and_2xCO2_spinup_361_commitf259310'
dire = testdir_in1
runmin=120
runmax=480
if (HPC == 'Yes') or (HPC == 'yes') or (HPC == 'y'):
...
precipitation3_avg_ctl = precipitation3_avg_ctl.assign_coords(lon=(((t_surf3.lon - 80) % 360))).sortby("lon")
precipitation3_avg = precipitation3_avg.assign_coords(lon=(((t_surf3.lon - 80) % 360))).sortby("lon")
net_lhe3_avg_ctl = net_lhe3_avg_ctl.assign_coords(lon=(((t_surf3.lon - 80) % 360))).sortby("lon")
net_lhe3_avg = net_lhe3_avg.assign_coords(lon=(((t_surf3.lon - 80) % 360))).sortby("lon")
...
landmaskxrA = landmaskxrA.assign_coords(lon=(((t_surf3.lon - 80) % 360))).sortby("lon")
...
axes[1,0].set_title('(c) Two continents flipped (2Cf)', size = med)
#fig = plt.figure()
...
# Two continents - Africa only 

array2 = (precipitation2_avg - precipitation2_avg_ctl)
array1 =  (precipitation3_avg - precipitation3_avg_ctl)
...
landmask1 = np.asarray(landmaskxrA)
landmask2 = np.asarray(landmaskxr2C)


axes[1,1].set_title('(d) 2Cf - AF', size = med)
#fig = plt.figure()
...
# array1, lons_cyclic = addcyclic(array1, lons)
# array1,lons_cyclic = shiftgrid(np.max(lons_cyclic)-180.,array1,lons_cyclic,start=False,cyclic=np.max(lons_cyclic))




array = np.asarray(array2) - np.asarray(array1)
...
fig.savefig('/scratch/mp586/Code/Graphics/Isca/ISCA_HPC/'+dire+'/Pavg_minus_ctl_4panels_2c_africa_america_P-Econts_120-480_nomerids.pdf', dpi=100)
